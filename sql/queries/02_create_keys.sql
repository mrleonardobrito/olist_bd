...
-- DESCRIBE olist.order;
ALTER TABLE olist.order ADD COLUMN id SERIAL;
DELETE FROM olist.order WHERE id IN (
    SELECT id FROM (
        SELECT order_id,
                id,
                ROW_NUMBER() OVER (PARTITION BY order_id, customer_id, order_approved_at ORDER BY order_id, customer_id, order_approved_at) AS rn
        FROM olist.order
    ) AS duplicates WHERE rn > 1
);
...
-- DESCRIBE order_item;
ALTER TABLE order_item DROP COLUMN order_item_id;
ALTER TABLE order_item ADD COLUMN order_item_id SERIAL;
...
UPDATE order_review SET review_id = UUID() WHERE id IN (
    SELECT id FROM (
        SELECT review_id,
               id,
...
ALTER TABLE order_review ADD CONSTRAINT pk_order_review PRIMARY KEY(review_id);